...
1) 沙盒包目录（外部）：
- Android/data/com.operit.backup/files/packages
2) 内置包：
...
const SANDBOX_EXTERNAL_PACKAGES_DIR = "/sdcard/Android/data/com.operit.backup/files/packages";
const TOOLPKG_DEBUG_INSTALL_ACTION = "com.operit.backup.DEBUG_INSTALL_TOOLPKG";
const TOOLPKG_DEBUG_INSTALL_COMPONENT =
  "com.operit.backup/.core.tools.packTool.ToolPkgDebugInstallReceiver";
const SANDBOX_SCRIPT_EXECUTION_ACTION = "com.operit.backup.EXECUTE_JS";
const SANDBOX_SCRIPT_EXECUTION_COMPONENT =
  "com.operit.backup/com.ai.assistance.operit.core.tools.javascript.ScriptExecutionReceiver";
const SANDBOX_SCRIPT_EXECUTION_MODE_SCRIPT = "script";
const SANDBOX_SCRIPT_EXECUTION_MODE_CODE = "code";
const SANDBOX_JS_TEMP_DIR = "/sdcard/Android/data/com.operit.backup/js_temp";
...
async function wait_for_android_file(path: string, timeoutMs: number): Promise<boolean> {
  const deadline = Date.now() + Math.max(0, timeoutMs);
...
function parse_json_text(raw: string): unknown | null {
...
async function debug_install_js_package(params?: {
  ...
  const finish = (payload: { success: boolean; message: string; data?: Record<string, unknown> }) =>
    complete({
      ...payload,
      data: {
        ...(payload.data ?? {}),
  ...
  try {
    ...
    logStep(
      `Install options -> enableAfterInstall=${String(enableAfterInstall)}, activateAfterInstall=${String(
    ...
    let enableResult: SandboxPackageUpdateResultData | null = null;
    if (shouldEnable) {
...
async function debug_install_toolpkg(params?: {
  ...
  try {
    ...
    logStep(
      `Install options -> resetSubpackageStates=${String(resetSubpackageStates)}, waitMs=${String(waitMs)}`
    );
    ...
    logStep(
      `Subpackage activation plan -> requested=${requestedSubpackages.join(", ") || "<none>"}, targets=${activationTargets.join(", ") || "<none>"}, unknown=${unknownRequestedSubpackages.join(", ") || "<none>"}`
    );

    const subpackageResults: Array<{
      subpackage_id: string;
      enable_result: SandboxPackageUpdateResultData;
...
async function debug_run_sandbox_script(params?: {
  ...
  try {
    ...
    logStep(
      `Resolved input -> sourcePath=${sourcePath || "<empty>"}, hasInlineCode=${String(hasInlineCode)}, waitMs=${String(waitMs)}`
    );
    ...
    logStep(
      `Direct execution tool completed -> success=${String(Boolean(executionResult.success))}, durationMs=${String(
...
type SpeechServicesConfigUpdateParams = {
  tts_service_type?: TtsServiceType;
  ...
  tts_pitch?: number;
  stt_service_type?: SttServiceType;
